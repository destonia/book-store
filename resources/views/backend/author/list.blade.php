...
@section('breadcrumb')
    <div class="row">
        ...
        <div class="nav-search col-sm-4" id="nav-search" style="text-align: right">
            <form class="form-search" action="{{route('authors.search')}}" method="post">
                @csrf
            ...
            <tr>
                ...
                <td class="center" style="padding-top: 20px">
                    <div class="hidden-sm hidden-xs btn-group">
                        <a class="btn btn-xs btn-info" href="{{route('authors.edit',$author->id)}}"><i
    ...
    <script language="JavaScript" type="text/javascript">
        $(document).ready(function () {
            $('.delete-button').click(function (e) {
                let authorName = $(this).closest('tr').find('#authorName').text()
                let authorId = $(this).closest('tr').find('#authorId').text()
                let authorAvatar = $(this).closest('tr').find('#authorAvatar').attr('src')
                $('#delete-name').text(authorName)
                $('#delete-id').val(authorId)
                $('#delete-avatar').attr('src',authorAvatar)
            });
        });
    </script>
@endsection
